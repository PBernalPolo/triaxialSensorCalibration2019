...
public class StorageManager implements Runnable {
  ...
  private double theta;  // angle of external servo [-pi,pi]
  private double phi;  // angle of internal servo [-pi/2,pi/2]
  private double temp;  // temperature of the senseHAT
  ...
  public StorageManager( PApplet aPApplet , TCB2_Manager theTCB ) {
  ...
  public synchronized void run() {
    while( this.running ){
      ...
      // and store data
      this.store_data();
      // finally, we check if everything is alright
      int sensorFailing = this.ISM.is_everythingAlright();
      if( sensorFailing != -1 ){
        System.out.println( "Sensor " + sensorFailing + " stopped working." );
        this.reset_serialPorts();
      }
      //
      try{
        while( this.wait ){
  ...
  public void reset_serialPorts() {
    System.out.println( "Resetting serial ports..." );
    for(int i=0; i<this.SPM.length; i++){
      this.SPM[i].stop();
    }
    this.NASPM = 0;
    this.update_serialPortManagers();
    this.ISM.reset_tLastMeasurement();
    System.out.println( "Serial ports reset." );
  }
  ...
  private short[] get_senseHatMeasurement() {
    ...
    // RqOI*RqIS*am/||am||=Ry*Rx=-g=(1,0,0)  =>  am/||am|| = ( c(theta) , s(theta)*s(phi) , s(theta)*c(phi) )  =>  phi = atan(y/z) ;  theta = signum(y)*atan(sqrt(y^2+z^2)/x)
    this.phi = Math.atan2( accelData[1] , accelData[2] )*(180.0/Math.PI);
    // we need to get phi in its interval
    this.phi = ( this.phi > 90.0 )? this.phi-180.0 : this.phi ;
    this.phi = ( this.phi < -90.0 )? this.phi+180.0 : this.phi ;
    // phi is in [-90º,90º]  =>  c(phi) >= 0  =>  signum(theta) = signum(s(theta)) = signum(a_z)
//    this.theta = Math.acos( accelData[0]/Math.sqrt( accelData[0]*accelData[0] + accelData[1]*accelData[1] + accelData[2]*accelData[2] ) )*Math.signum( accelData[2] )*(180.0/Math.PI);
    this.theta = Math.atan2( Math.sqrt( accelData[1]*accelData[1] + accelData[2]*accelData[2] ) , accelData[0] )*Math.signum( accelData[2] )*(180.0/Math.PI);
  ...
  private void store_data() {
    ...
    while( this.ISM.available() ){
      ...
      byte[] theBytes = this.ISM.get_measurement();
      if( theBytes[0] == 100 ){
        long[] m = this.get_measurementP( theBytes );
        ...
        int[] m = this.get_measurement( theBytes );
        // and we write it to the file only if it is not null
        if(  m != null  &&  this.storeMeasurements  ){
          this.PW.print( this.Nm );
          for(int i=0; i<m.length; i++){
    ...
    short[] m = this.get_senseHatMeasurement();  // we always take the measurement to update the angles and the temperature
    if( this.storeMeasurements ){
      this.PW.print( this.Nm + " " + 17 + " " );  // WHO_AM_I
      for(int i=0; i<m.length; i++){
        this.PW.print( m[i] + " " );
      }
      this.PW.println( this.TCB.get_angularVelocity() + " " + this.presTemp.get_pressureData() + " " + this.presTemp.get_temperatureData() );
    }
  }
  ...
  private long[] get_measurementP( byte[] theBytes ) {
    ...
    //theBytes[0];  // INFORMATION_SOURCE_CLASS_ID
    m[0] = theBytes[1];  // WHO_AM_I
    m[1] = ( ( theBytes[5] << 24 ) | ( (theBytes[4] & 0xFF) << 16 ) | ( (theBytes[3] & 0xFF) << 8 ) | (theBytes[2] & 0xFF) );  // tempP
    m[2] = ( ( theBytes[9] << 24 ) | ( (theBytes[8] & 0xFF) << 16 ) | ( (theBytes[7] & 0xFF) << 8 ) | (theBytes[6] & 0xFF) );  // mp
    return m;
  }
  ...
  private void senseHatSetup() {
    ...
    // settings:
    // measurement range settings
    this.magnetometer.set_magnetometerMeasurementRange( 0 );  // ( mode=0 -> 4gauss;  mode=1 -> 8gauss;  mode=2 -> 12gauss;  mode=3 -> 16gauss )
    // data rate settings
    ...
    // object creation:
    // if you do not know the device direction, simply use
    //  this.presTemp = new LPS25H();
    // however, if you know the device direction it will be faster (and cleaner if you look at the console) to use
    this.presTemp = new LPS25H( 0x5C );
    
    // some possible operations:
    this.presTemp.reset();  // to reset the device using software
    this.presTemp.reboot();  // to reboot the memory of the device
    // power-off the device
    this.presTemp.powerOff();
    // power-on the device
    this.presTemp.powerOn();
    
    // settings:
    // pressure resolution
    this.presTemp.set_pressureResolution( 8 );  // number of samples taken to average. The interval is [8,512]
    // temperature resolution
    this.presTemp.set_temperatureResolution( 8 );  // number of samples taken to average. The interval is [8,64]
    // data rate settings
    this.presTemp.set_outputDataRate( 4 );  // ( mode=0 -> one-shot;  mode=1 -> 1Hz;  mode=2 -> 7Hz;  mode=3 -> 12.5Hz;  mode=4 -> 25Hz )
    // update operation
    this.presTemp.set_nonContinuousDataUpdate();  // (this feature prevents the reading of LSB and MSB related to different samples)
